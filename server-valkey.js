...
require('dotenv').config({ path: '.env.valkey' });
...
// === VALKEY OFFICIAL CLIENT ===
const valkeyClient = require('./src/cache/valkeyClient');
const cache = valkeyClient(process.env.CACHE_HOST, process.env.CACHE_PORT);
...
const init = async () => {
  ...
  await server.start();
  console.log("Server WITH VALKEY CACHE running on", server.info.uri);
};
